...
export default function Logo({ size = 'md', className = '' }) {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      >
        <path d="M16 2 L28 9 V23 L16 30 L4 23 V9 Z" className="stroke-primary" strokeWidth="2" />
        <path
      ...
      <div className="leading-tight">
        <p className={`${s.title} font-display font-bold tracking-tight text-foreground`}>
          RENDER CUBE
        </p>
        <p className={`${s.subtitle} text-foreground-soft`}>Digital Assets Marketplace</p>
      </div>
    </div>
  );
}
