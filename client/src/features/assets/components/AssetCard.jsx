  ...
  return (
    ...
    >
      <div className={clsx('relative aspect-[4/3] overflow-hidden bg-linear-to-br', asset.color)}>
        {/* Hover scrim — a plain darken is intentional here, this sits on top of
            arbitrary thumbnail art rather than themed UI chrome. */}
        <div className="absolute inset-0 bg-black/0 transition-colors duration-300 group-hover:bg-black/20" />

        {asset.free && (
          <span className="glass absolute left-2.5 top-2.5 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
            Free
          </span>
        )}
        ...
        <button
          ...
          className={clsx(
            'absolute right-2.5 top-2.5 flex h-7 w-7 items-center justify-center rounded-full transition-all duration-200',
            saved
              ? 'bg-rose text-white opacity-100'
              : 'glass text-white opacity-0 group-hover:opacity-100 hover:brightness-110'
          )}
