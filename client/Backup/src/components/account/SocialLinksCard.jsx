  ...
  return (
    <Card className="p-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        {onEdit && (
          <button onClick={onEdit} className="text-sm font-medium text-violet hover:text-fuchsia">
            Edit
          </button>
        )}
      </div>
        ...
        {social.map((s) => {
          ...
          return (
            <li key={s.platform}>
              <a
                href="#"
                className="flex items-center gap-3 rounded-xl px-2 py-2 text-sm transition-colors hover:bg-surface-raised"
              >
                <span
                ...
                <span className="min-w-16 font-medium text-ink">{s.platform}</span>
                <span className="flex-1 truncate text-ink-faint">{s.handle}</span>
